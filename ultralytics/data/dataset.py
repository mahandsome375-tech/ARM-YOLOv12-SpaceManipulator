...
class YOLODataset(BaseDataset):
    ...
    def __getitem__(self, index):
        """
        Override BaseDataset __getitem__ to add depth map and extra supervision.
        """
        import os
        import cv2
        import numpy as np
        ...
        H, W = sample["img"].shape[1], sample["img"].shape[2]
        stem = sample["im_file"].rsplit(".", 1)[0]
        ...
        def _try_load_npy(p, shape=None, default=0.0):
            if os.path.exists(p):
                arr = np.load(p).astype(np.float32)
                if shape is not None and arr.shape != shape:
                    arr = cv2.resize(arr, (shape[1], shape[0]), interpolation=cv2.INTER_NEAREST)
                return arr
            return np.full(shape if shape is not None else (1,), float(default), dtype=np.float32)

        def _try_load_gray(p, shape, default=0.0):
            if os.path.exists(p):
        ...
        phi_sin = _try_load_npy(stem + "_phi_sin.npy", (H, W))
        phi_cos = _try_load_npy(stem + "_phi_cos.npy", (H, W))
        tau = _try_load_gray(stem + "_tau.png", (H, W))
        logvar = _try_load_npy(stem + "_logvar.npy", (H, W))
        valid = _try_load_npy(stem + "_valid.npy", (H, W), default=1.0)

        sample["phi_sin"] = torch.from_numpy(phi_sin).unsqueeze(0)
        sample["phi_cos"] = torch.from_numpy(phi_cos).unsqueeze(0)
        sample["tau"] = torch.from_numpy(tau).unsqueeze(0)
        sample["logvar"] = torch.from_numpy(logvar).unsqueeze(0)
        sample["valid_mask"] = torch.from_numpy(valid).unsqueeze(0)
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"img", "text_feats"}:
                ...
                value = [v if isinstance(v, torch.Tensor)
                         else torch.as_tensor(v, dtype=torch.float32) for v in value]
                value = torch.stack(value, 0)

            elif k in {"phi_sin", "phi_cos", "tau", "logvar", "valid_mask"}:
