...
"""Model head modules."""
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment", "DDetect"
...
class DDetect(Detect):
    def __init__(self, nc: int = 80, ch: tuple = ()):
        super().__init__(nc, ch)

        c4_list = [max(c // 4, 1) for c in ch]
        self.cv4 = nn.ModuleList(
            ...
            for c, c4 in zip(ch, c4_list)
        )

        cphi_list = [max(c // 4, 2) for c in ch]
        self.cv_phi = nn.ModuleList(
            nn.Sequential(Conv(c, cp, 3), Conv(cp, cp, 3), nn.Conv2d(cp, 2, 1))
            for c, cp in zip(ch, cphi_list)
        )

        ctau_list = [max(c // 4, 1) for c in ch]
        self.cv_tau = nn.ModuleList(
            nn.Sequential(Conv(c, ct, 3), Conv(ct, ct, 3), nn.Conv2d(ct, 1, 1))
            for c, ct in zip(ch, ctau_list)
        )

        csig_list = [max(c // 4, 1) for c in ch]
        self.cv_logvar = nn.ModuleList(
            nn.Sequential(Conv(c, cs, 3), Conv(cs, cs, 3), nn.Conv2d(cs, 1, 1))
            for c, cs in zip(ch, csig_list)
        )

    def forward(self, x: list[torch.Tensor], *args, return_extra: bool = False, **kwargs):
        """
        By default, this is consistent with the original Detect behavior and returns tensors/tuples for model.predict.
        When return_extra=True, return a dict containing {"det":..., "feats":..., "depth":...}
        """
        bs = x[0].shape[0]


        d_per_level = [self.cv4[i](x[i]) for i in range(self.nl)]
        H3, W3 = d_per_level[0].shape[-2:]
        depth_fused = d_per_level[0]
        for i in range(1, self.nl):
            depth_fused = depth_fused + F.interpolate(d_per_level[i],
                                                      size=(H3, W3),
                                                      mode="bilinear",
                                                      align_corners=False)
        depth_fused = F.softplus(depth_fused)
        self.last_depth = depth_fused


        phi_per_level = [self.cv_phi[i](x[i]) for i in range(self.nl)]
        tau_per_level = [self.cv_tau[i](x[i]) for i in range(self.nl)]
        logvar_per_level = [self.cv_logvar[i](x[i]) for i in range(self.nl)]

        H3, W3 = depth_fused.shape[-2:]

        def fuse_to_p3(feat_list):
            fused = F.interpolate(feat_list[0], size=(H3, W3), mode="bilinear", align_corners=False)
            for i in range(1, self.nl):
                fused = fused + F.interpolate(feat_list[i], size=(H3, W3), mode="bilinear", align_corners=False)
            return fused

        phi_fused = fuse_to_p3(phi_per_level)
        eps = 1e-6
        norm = torch.sqrt(phi_fused[:, 0:1] ** 2 + phi_fused[:, 1:2] ** 2 + eps)
        phi_unit = phi_fused / norm

        tau_fused = F.softplus(fuse_to_p3(tau_per_level))
        logvar_fused = torch.clamp(fuse_to_p3(logvar_per_level), -8, 2)


        self.last_depth = depth_fused
        self.last_phi = phi_unit
        self.last_tau = tau_fused
        self.last_logvar = logvar_fused
        ...
        if getattr(self, "export", False) and not return_extra:
        ...
        if isinstance(out, tuple):
            infer_pred, feats = out
        else:
            infer_pred, feats = out, None
        return {
            "det": infer_pred,
            "feats": feats,
            "depth": depth_fused,
