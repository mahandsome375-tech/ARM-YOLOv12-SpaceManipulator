...
class DummyEMA:
    """A safe dummy EMA class used when EMA is disabled."""
    def __init__(self):
        self.ema = None
...
class BaseTrainer:
    ...
    def _setup_train(self):
        ...
        if RANK in {-1, 0}:
            ...
            try:
                ...
                LOGGER.warning(f"⚠️ EMA disabled due to error: {e}")
                ...
                class DummyEMA:
                    ...
                    def update_attr(self, *args, **kwargs): pass
        ...
        torch.save(
            {
                ...
                "ema": (deepcopy(unwrap_model(self.ema.ema)).half()
                        if getattr(self, "ema", None) is not None and getattr(self.ema, "ema", None) is not None else None),
    ...
    def preprocess_batch(self, batch):
        ...
        if "depth" in batch and batch["depth"] is not None:
            d = batch["depth"].to(self.device, non_blocking=True, dtype=batch["img"].dtype)
            if d.ndim == 3:
                d = d.unsqueeze(1)
            if d.max() > 1:
                d = d / 255.0
            batch["img"] = torch.cat([batch["img"], d], dim=1)


            if not hasattr(self, "_printed_depth"):
                print(f"✅ RGBD batch shape: {batch['img'].shape}")
