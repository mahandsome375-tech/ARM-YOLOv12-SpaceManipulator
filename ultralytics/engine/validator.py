...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            model = getattr(ema_obj, "ema", None) or getattr(trainer, "model", None) or model
    ...
    def preprocess(self, batch):
        """Move images to device, cast to the right dtype, and normalize to [0,1]."""
        im = batch["img"].to(self.device, non_blocking=True)
        im = im.half() if self.args.half else im.float()
        im /= 255.0
        batch["img"] = im
        return batch
